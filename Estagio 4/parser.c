...
Declaracao* AnaliseDeclaracao() {
    ...
    if (t->tipo != TOKEN_VAR && t->tipo != TOKEN_INT )
        return NULL;
...
Programa* AnalisePrograma() {
    ...
    if (t->tipo != TOKEN_PRINT && t->tipo != TOKEN_PRINTF && t->tipo != TOKEN_PRINTINT) {
...
Expressao* AnaliseExpressao() {
    ...
    if (t->tipo != TOKEN_SOMA && t->tipo != TOKEN_SUB && t->tipo != TOKEN_MULT && t->tipo != TOKEN_DIV) {
...
void DestroiExpressao(Expressao *e) {
    if (e->oper == OPER_SOMA || e->oper == OPER_SUB || e->oper == OPER_MULT || e->oper == OPER_DIV) {
